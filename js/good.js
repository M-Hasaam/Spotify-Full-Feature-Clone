...
(async function main() {
  ...
  albums.forEach((album) => {
    ...
    cardContainer.innerHTML += `
      <div class="card cursor" data-folder="songs/${folder}/" data-title="${album.title}" data-description="${album.description}">
  ...
  function waitForFirstCard() {
    ...
    if (!firstAlbum || !firstAlbum.songs.length) return;

    songList = firstAlbum.songs.map((song) => `songs/${firstAlbum.folder}/${song}`);

    SongContainer.innerHTML = "";
    firstAlbum.songs.forEach((song) => {
      const songName = decodeURIComponent(song).replaceAll("/", "").replaceAll("%20", " ");
      SongContainer.innerHTML += `
        <li class="song_list cursor" data-folder="songs/${firstAlbum.folder}/${song}">
